...
export class KsDashboarditemtype extends Component {
        ...
        onselectitemtype(ev){
         ...
         ks_value = ks_value.trim(' ');
         var ks_target =  this.state.charts.find((item)=>{
            return item.name === ks_value
         })
        this.props.update(ks_target['Tech_name'])
        }

    }
KsDashboarditemtype.template="ks_dashboard_item_view_owl";
KsDashboarditemtype.supportedTypes = ["char"];
registry.category("fields").add('ks_dashboard_item_type', KsDashboarditemtype);
