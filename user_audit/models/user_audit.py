...
class UserAudit(models.Model):
    ...
    is_read = fields.Boolean(string="Read", help="Enabling we can track all read activities It will track all your read activity that may increase the size of the log that may cause some problem with your data base")
    is_write = fields.Boolean(string="Write", help="Enabling we can track all write activities")
    is_create = fields.Boolean(string="Create", help="Enabling we can track all create activities")
    is_delete = fields.Boolean(string="Delete", help="Enabling we can track all delete activities")
    is_all_users = fields.Boolean(string="All Users", help="Enabling we can track activities of all users")
    user_ids = fields.Many2many('res.users', string="Users", help="Manage users")
    model_ids = fields.Many2many('ir.model', string="Model", help='Used to select which model is to track')
