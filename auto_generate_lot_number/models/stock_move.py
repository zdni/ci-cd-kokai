...
class StockMove(models.Model):
    ...
    def _prepare_move_line_vals(self, quantity=None, reserved_quant=None):
        ...
        if auto_generate:
            ...
            if serial_number_type == 'global':
                vals.update({ 'lot_name': self.env['ir.sequence'].next_by_code('res.config.settings') })
            else:
    ...
    def action_show_details(self):
        ...
        if auto_generate:
            view = self.env.ref('auto_generate_lot_number.view_stock_move_nosuggest_operations_custom')
            res.update({
                'views': [(view.id, 'form')],
                'view_id': view.id,
            })
